...
This document provides a summary of *R* concepts that will be useful to learn or refresh in preparation for this "Modelling for Pandemic Preparedness and Response" in-person course.

There are links in various places which will take you to web sites that provide further information, if you would like more detail on any particular concept.

A good general and detailed introduction to *R* is provided in the [R for Data Science](https://r4ds.hadley.nz/) by Hadley Wickham and others.

## Interacting with this tutorial

This tutorial is written in [Quarto](https://quarto.org/) and can be rendered to various formats including HTML and PDF.

To render this document, you will need the latest version of [R](https://www.r-project.org/) and [Quarto](https://quarto.org/docs/download/), and an Integrated Development Environment (IDE) such as [RStudio](https://posit.co/download/rstudio-desktop/), [Positron](https://positron.posit.co/download.html) or [VSCode](https://code.visualstudio.com/).

You can run the code chunks interactively in your IDE or render the entire document by clicking the "Render" next to the magnifying glass above. From there, you can copy and paste the code into your console.

::: callout-tip
### Tip

For the best experience, open this tutorial in R Studio and switch from *Source* to *Visual* mode on the top left. You can then interact with the code by clicking the green arrow next to each code chunk.
:::

## Objectives

The aim of this tutorial is to introduce you to the basics of R programming.

This tutorial is organised in three parts from the basics of *R* through intermediate *R* to how to use *R* to build compartmental models of infectious disease dynamics.
...
Within R code, comments are created with `#` or `#'`. Anything written after these two are "commented" out and won't be treated as R code. It is good coding practice to comment your code as you go through.
...
There are two ways to do this: 1) In RStudio, go to the "Packages" tab and search for the required package and load it. 2) Try replacing "packagename" with the name of the package you would like to install.
...
Can you install the package `deSolve`? We will use this to solve ordinary differential equations later on in this practical.
...
You need to tell the R session where you want it to work from. To do this you use the command `setwd()` to "set the working directory".
...
Did you get an error when you tried to load the `here` package?

Hint: It most likely means you need to install it first using `install.packages("here")`.
:::
...
Vectors are an ordered collection of simple elements such as numbers or strings. They can be created with the concartenate or, `c()` function.
...
To create a sequence along the length of a vector, use `seq_along()`. This is useful for looping over the elements of a vector, for example, when exploring parameters.
...
Lists are different from vectors in that elements of a list can be of any type (including more lists, vectors, etc.), and not all elements have to be of the same type either.
...
Data frames are 2-dimensional (only have rows and columns) extensions of vectors. They can be thought of as the *R*-version of an Excel spreadsheet.
...
Here is how to create a matrix of 0's.

::: callout-note
#### Question

What do the `4` and `2` arguments represent?
:::
...
Note the use of the `here` package. Do you understand how it works?
:::
...
What will happen to `mm` over time? Be careful not to overwrite things.
:::
...
Can you understand what the required inputs are?
:::
